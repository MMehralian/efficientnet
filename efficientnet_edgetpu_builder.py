...
def main():
    ...
    normalized_features = tf.zeros([16, 224, 224, 3], dtype=tf.dtypes.float32, name='image')
    mean_rgb = MEAN_RGB
    stddev_rgb = STDDEV_RGB
    stats_shape = [1, 1, 3]

    normalized_features -= tf.constant(mean_rgb, shape=stats_shape, dtype=tf.float32)
    normalized_features /= tf.constant(stddev_rgb, shape=stats_shape, dtype=tf.float32)

    override_params = {'data_format': 'channels_last', 'num_classes': 1000}

    print(normalized_features)

    logits, model = build_model(normalized_features, model_name='efficientnet-edgetpu-S', training=True,
                                override_params=override_params, model_dir='EfficientNet_Esge_S')
